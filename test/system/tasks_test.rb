require "application_system_test_case"

class TasksTest < ApplicationSystemTestCase
  setup do
    @task = tasks(:one)
  end

  test "visiting the index" do
    visit tasks_url
    assert_selector "h1", text: "Tasks"
  end

  test "creating a Task" do
    visit tasks_url
    click_on "New Task"

    fill_in "Content", with: @task.content
    fill_in "End date", with: @task.end_date
    fill_in "Expect time", with: @task.expect_time
    fill_in "Progress rate", with: @task.progress_rate
    fill_in "Result time", with: @task.result_time
    fill_in "Start date", with: @task.start_date
    fill_in "Task", with: @task.task_id
    fill_in "Task kind", with: @task.task_kind_id
    fill_in "Task name", with: @task.task_name
    fill_in "Task status", with: @task.task_status
    fill_in "User", with: @task.user_id
    click_on "Create Task"

    assert_text "Task was successfully created"
    click_on "Back"
  end

  test "updating a Task" do
    visit tasks_url
    click_on "Edit", match: :first

    fill_in "Content", with: @task.content
    fill_in "End date", with: @task.end_date
    fill_in "Expect time", with: @task.expect_time
    fill_in "Progress rate", with: @task.progress_rate
    fill_in "Result time", with: @task.result_time
    fill_in "Start date", with: @task.start_date
    fill_in "Task", with: @task.task_id
    fill_in "Task kind", with: @task.task_kind_id
    fill_in "Task name", with: @task.task_name
    fill_in "Task status", with: @task.task_status
    fill_in "User", with: @task.user_id
    click_on "Update Task"

    assert_text "Task was successfully updated"
    click_on "Back"
  end

  test "destroying a Task" do
    visit tasks_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Task was successfully destroyed"
  end
end
